	...
	for i := 1; i <= 10; i++ {
		...
		for t := 0; t < 10; t++ {
			db.Rooms = append(db.Rooms,
				models.Room{ID: roomID, HotelID: i, Name: fmt.Sprintf("VIP-%02d", t+1), Type: models.RoomVIP, PricePerNight: 2200000 + t*25000, Beds: 2, Capacity: 4, Stock: 5, Facilities: []string{"Private Jacuzzi", "Smart TV 65\"", "Lounge Access", "Mini Bar", "Bathtub"}, FloorMapImage: "https://images.unsplash.com/photo-1611892440504-42a792e24d32?auto=format&fit=crop&w=1000&q=80"},
				models.Room{ID: roomID + 1, HotelID: i, Name: fmt.Sprintf("DELUXE-%02d", t+1), Type: models.RoomDeluxe, PricePerNight: 1450000 + t*20000, Beds: 2, Capacity: 3, Stock: 5, Facilities: []string{"City View", "King Bed", "Work Desk", "Rain Shower", "Sofa"}, FloorMapImage: "https://images.unsplash.com/photo-1631049307264-da0ec9d70304?auto=format&fit=crop&w=1000&q=80"},
				models.Room{ID: roomID + 2, HotelID: i, Name: fmt.Sprintf("REG-%02d", t+1), Type: models.RoomRegular, PricePerNight: 780000 + t*15000, Beds: 1, Capacity: 2, Stock: 5, Facilities: []string{"AC", "WiFi", "TV 42\"", "Breakfast", "Shower"}, FloorMapImage: "https://images.unsplash.com/photo-1590490360182-c33d57733427?auto=format&fit=crop&w=1000&q=80"},
			)
			roomID += 3
		}
	}
