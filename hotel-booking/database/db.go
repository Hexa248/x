...
func Seed() *InMemoryDB {
	...
	for cityIdx, city := range cities {
		for i := 0; i < 15; i++ {
			...
			hotel := models.Hotel{
				ID:          hotelID,
				Name:        fmt.Sprintf("%s %s %d", hotelPrefixes[i%len(hotelPrefixes)], city, i+1),
				City:        city,
			...
			db.Rooms = append(db.Rooms,
				models.Room{ID: roomID, HotelID: hotelID, Name: fmt.Sprintf("VIP-%02d", i+1), Type: models.RoomVIP, PricePerNight: 2200000 + (i * 30000), Beds: 2, Capacity: 4, Stock: 5, Facilities: vipFacilities[(cityIdx+i)%len(vipFacilities)], FloorMapImage: "https://images.unsplash.com/photo-1611892440504-42a792e24d32?auto=format&fit=crop&w=1000&q=80"},
				models.Room{ID: roomID + 1, HotelID: hotelID, Name: fmt.Sprintf("DELUXE-%02d", i+1), Type: models.RoomDeluxe, PricePerNight: 1400000 + (i * 25000), Beds: 2, Capacity: 3, Stock: 5, Facilities: deluxeFacilities[(cityIdx+i)%len(deluxeFacilities)], FloorMapImage: "https://images.unsplash.com/photo-1631049307264-da0ec9d70304?auto=format&fit=crop&w=1000&q=80"},
				models.Room{ID: roomID + 2, HotelID: hotelID, Name: fmt.Sprintf("REG-%02d", i+1), Type: models.RoomRegular, PricePerNight: 700000 + (i * 20000), Beds: 1, Capacity: 2, Stock: 5, Facilities: regularFacilities[(cityIdx+i)%len(regularFacilities)], FloorMapImage: "https://images.unsplash.com/photo-1590490360182-c33d57733427?auto=format&fit=crop&w=1000&q=80"},
			)
